...
let set_ecall_func (tf : trusted_func) =
    ...
    [
        ...
        "    if (!in_buf || !cc_enclave_memory_in_enclave(in_buf, in_buf_size))";
        "        goto done;";
        "";
        "    if (out_buf && !cc_enclave_memory_in_enclave(out_buf, out_buf_size))";
        "        goto done;";
        "";
        "    /* Call host function */";
        "    " ^ concat "    \n" (CommonfuncPL.set_call_user_func tfd);
        "    /* Sucess */";
        "    result = CC_SUCCESS;";
...
let set_main_func =
    [
        ...
        "    uint8_t* in_buf = NULL;";
        "    uint8_t* out_buf = NULL;";
        "    uint8_t* in_buf_umem_ptr = NULL;";
        "    uint8_t* out_buf_umem_ptr = NULL;";
        "    size_t output_bytes_written = 0;";
        "    untrusted_mem_info_t *mem_info = ";
        "        (untrusted_mem_info_t *)DEFAULT_UNTRUSTED_PTR;";
        "    size_t copy_untrusted_mem_size = ";
        "        size_to_aligned_size(sizeof(untrusted_mem_info_t)) + ";
        "        mem_info->in_buf_size + mem_info->out_buf_size;";
        "";
        "    fid = mem_info->fid;";
        "    eapp_print(\"ecall function id: %d\\n\", fid);";
        "    if(fid >= ecall_table_size){";
        "        eapp_print(\"Can't find ecall function \\n\");";
        "        EAPP_RETURN(0);";
        "    }";
        ...
        "    }";
        "";
        "    in_buf = (uint8_t*)malloc(mem_info->in_buf_size);";
        "    in_buf_umem_ptr = (uint8_t*)DEFAULT_UNTRUSTED_PTR + ";
        "        size_to_aligned_size(sizeof(untrusted_mem_info_t));";
        "    memcpy(in_buf, in_buf_umem_ptr, mem_info->in_buf_size);";
        "";
        "    if(mem_info->out_buf_size > 0){";
        "        out_buf = (uint8_t*)malloc(mem_info->out_buf_size);";
        "        out_buf_umem_ptr = in_buf_umem_ptr + mem_info->in_buf_size;";
        "        memcpy(out_buf, out_buf_umem_ptr, mem_info->out_buf_size);";
        "    }";
        "";
        "    cc_ecall_tables[fid](in_buf, mem_info->in_buf_size, ";
        "        out_buf, mem_info->out_buf_size, &output_bytes_written);";
        "    eapp_print(\"ecall output_bytes_written: %d\\n\", output_bytes_written);";
        "";
...
let gen_trusted(ec : enclave_content) =
    ...
    let main_func = set_main_func in
    let ecall_table =
        [
    ...
    [
        "";
        sprintf "#include \"%s_t.h\"\n#include \"eapp.h\"" ec.file_shortnm;
        "";
        "#include <stdio.h>";
        "#include <string.h>";
        "";
