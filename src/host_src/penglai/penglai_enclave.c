...
cc_enclave_result_t _penglai_create(cc_enclave_t *enclave,
  ...
  if (features != NULL || features_count > 0) {
    print_error_term(
        "Penglai does not currently support additional features\n");
...
cc_enclave_result_t handle_ocall(struct PLenclave *penglai_enclave,
                                 uint8_t *untrusted_mem_extent,
                                 const void *ocall_table, int *result) {
  ...
  if (ocall_func_id >= ocall_table_size) {
    print_debug("[ERROR] host: ocall function isn't exist!, %d >= %d\n", ocall_func_id, ocall_table_size);
    return CC_FAIL;
  }

  ocall_in_buf =
      untrusted_mem_extent + size_to_aligned_size(sizeof(untrusted_mem_info_t));
  ocall_out_buf = ocall_in_buf + ocall_mem_info->in_buf_size;
  if (((ocall_enclave_table_t *)ocall_table)
          ->ocalls[ocall_func_id](ocall_in_buf, ocall_mem_info->in_buf_size,
                                  ocall_out_buf,
...
cc_enclave_result_t
cc_enclave_call_function(cc_enclave_t *enclave, uint32_t function_id,
  ...
  while (result != 0) {
    if (result == RETURN_USER_FOR_OCALL) {
      result_cc = handle_ocall(penglai_enclave, untrusted_mem_extent,
                               ocall_table, &result);
...
cc_enclave_result_t cc_tee_registered(cc_enclave_t *context, void *handle) {
  ...
  if (OPS_NAME.type_version != context->type ||
      OPS_NODE.ops_desc != &OPS_NAME || len >= MAX_ENGINE_NAME_LEN ||
      OPS_NAME.ops != &OPS_STRU) {
...
cc_enclave_result_t cc_tee_unregistered(cc_enclave_t *context,
                                        enclave_type_version_t type_version) {
  if (context == NULL || context->list_ops_node != &OPS_NODE ||
      type_version != OPS_NAME.type_version) {
