...
CC_API_SPEC list_ops_management g_list_ops = {.count = 0,
                                              .pthread_flag = false,
                                              .mutex_work =
...
static err2str g_secgearerror[] = {
    ...
    {CC_ERROR_ENCLAVE_MAXIMUM,
     "The number of connections or enclave reaches the maximum."},
    ...
    {CC_ERROR_INVALID_CPUSVN,
     "The cpu svn is beyond platform's cpu svn value."},
    {CC_ERROR_INVALID_ISVSVN,
    ...
    {CC_ERROR_STORAGE_EAGAIN,
     "Secure storage partition is unavailable, try again."},
    {CC_ERROR_STORAGE_ENOTDIR, "Operation object is not a directory."},
    {CC_ERROR_STORAGE_EISDIR, "Operation object is a directory."},
    {CC_ERROR_STORAGE_ENFILE,
     "The number of open files reaches the maximum of system."},
    {CC_ERROR_STORAGE_EMFILE,
     "The number of open files reaches the maximum of process."},
    ...
    {CC_ERROR_STORAGE_NOT_AVAILABLE,
     "Secure storage partition is unavailable."},
...
cc_enclave_result_t find_engine_open(enclave_type_version_t type,
                                     void **handle) {
...
cc_enclave_result_t find_engine_registered(void *handle,
                                           p_tee_registered *p_func,
                                           p_tee_unregistered *p_unfun) {
...
enclave_type_version_t match_tee_type_version(enclave_type_t type,
                                              uint32_t version) {
  ...
  default:
    print_error_term(
        "Detection platform type error: only support aarch64 and x86_64\n");
    return ENCLAVE_TYPE_VERSION_MAX;
  }
}
...
static void remove_ops(struct list_ops_desc *fp,
                       const struct list_ops_desc *p) {
  if (fp == NULL) {
